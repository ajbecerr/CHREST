...
def MakeModFile(PROJECT_DIR, ABLATE_ARCH, ABLATE_DIR, DEBUG, AVX512):
  ...
  for m in MODS:
    if m.find('salac')==-1 and m.find('petscXdmf')==-1 and m.find('chrest')==-1:
      f.write('prereq("'+m+'")\n')
...
if __name__ == '__main__':
  ...
  cmd.append('-DCMAKE_C_COMPILER='+I_MPI_ROOT+'/intel64/bin/mpicc')
  cmd.append('-DCMAKE_CXX_COMPILER='+I_MPI_ROOT+'/intel64/bin/mpicxx')

  #cmd.append('-DCMAKE_C_COMPILER='+OMPI_DIR+'/bin/mpicc')
  #cmd.append('-DCMAKE_CXX_COMPILER='+OMPI_DIR+'/bin/mpicxx')
