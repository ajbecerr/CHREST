...
#  os.system('perl -pi -e \'s/\\x27\\\\n\\x27.join/\\x27 \\x27.join/g\' ./config/BuildSystem/config/packages/tchem.py')
  ...
  HDF_DIR = os.getenv('HDF5')
  LIB_DIR = os.getenv('LIB_DIR')
  I_MPI_ROOT = os.getenv('I_MPI_ROOT')
  MKL_DIR = os.getenv('MKL')
  OCE_DIR = os.getenv('OCE_DIR')
  MAKE_DIR = os.getenv('MAKE_DIR')
  OMPI_DIR = os.getenv('OMPI')
  HDF5_DIR = os.getenv('HDF5')
  ...
  PETSC_ARCH = GetPetscArch(DEBUG, AVX512)
    ...
    #else:
      #coptflags = '-g -O3 -fp-model fast=2'

  if(DEBUG==1):
    coptflags = '-g -O0'
  else:
    if(AVX512==1):
      coptflags = '-g -O3 -xCORE-AVX512'
    else:
      coptflags = '-g -O3'


#'--with-cc='+I_MPI_ROOT+'/intel64/bin/mpiicc',
    #'--with-cxx='+I_MPI_ROOT+'/intel64/bin/mpiicpc',
  ...
  configure_options = [
    ...
    '--with-fc='+I_MPI_ROOT+'/intel64/bin/mpif90',
    #'--with-cc='+OMPI_DIR+'/bin/mpicc',
    #'--with-cxx='+OMPI_DIR+'/bin/mpicxx',
    #'--with-fc='+OMPI_DIR+'/bin/mpif90',
    #'--with-hdf5=1',
    #'--with-hdf5-dir='+HDF5_DIR,
    '--download-hdf5',
    '--download-hdf5-configure-arguments=--enable-build-mode=production --enable-parallel',
    '--download-ctetgen',
    '--download-egads',
    '--download-metis',
    '--download-ml',
    ...
    '--download-tchem',
    #'--with-opencascade=1',
    #'--with-opencascade-dir='+OCE_DIR,
    '--download-opencascade',
    '--download-cmake=1',
    '--download-make=1',
    #'--with-make-dir='+MAKE_DIR,
    '--download-googletest=0',
    '--download-triangle',
  ...
  cmd.append('-R');
  cmd.append('755')
  cmd.append(LIB_DIR+"/petsc/"+PETSC_ARCH)
